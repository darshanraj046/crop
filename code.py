...
account_sid = 'AC9262e2c2436aecea664c757c6dfe56bf'
auth_token = '383997d6776728a89b254d2e41cf86eb'
...
def main():
    ...
    with col2:
        ...
        # Display the values sent from ESP32
        if 'esp32_data' in st.session_state:
            N = st.number_input("Nitrogen", value=st.session_state.esp32_data['Nitrogen'], min_value=0, max_value=250)
            P = st.number_input("Phosphorus", value=st.session_state.esp32_data['Phosphorus'], min_value=0,
                                max_value=250)
            K = st.number_input("Potassium", value=st.session_state.esp32_data['Potassium'], min_value=0, max_value=250)
            temp = st.number_input("Temperature", value=st.session_state.esp32_data['Temperature'], min_value=0.0,
                                   max_value=100.0)
            humidity = st.number_input("Humidity in %", value=st.session_state.esp32_data['Humidity'], min_value=0.0,
                                       max_value=250.0)
        else:
        ...
        crop_encoded = np.array([1 if c == (prediction.item()) else 0 for c in
        ...
        if st.button('Predict'):
            ...
            message = client.messages \
                .create(
                body=a + " are recommended for your farm with yield of {:.3f}.".format(prediction1[0]),
                from_='+13613664534',
                to='+919036310656'
            )
            col1.write('''
